...
app.post("/register", async (req, res) => {
	...
	try {
		...
		req.session.error = {
			err_level: "danger",
			err_msg:
				"Failed to register user. Please try again. Error: " +
					error.message || error,
		};
...
app.post("/information", async (req, res) => {
	...
	try {
		await db.none(query, [
			...
			req.body.college,
			req.body.degree,
			req.body.high_school,
			req.body.company_name,
			req.body.company_name_2,
			req.body.company_name_3,
			req.body.company_name_4,
			req.body.position,
			req.body.position_2,
			req.body.position_3,
			req.body.position_4,
			req.body.employment_time,
			req.body.employment_time_2,
			req.body.employment_time_3,
			req.body.employment_time_4,
			req.body.achievements,
			req.body.achievements_2,
			req.body.achievements_3,
			req.body.achievements_4,
			req.body.skills,
			req.body.referal_name,
			req.body.referal_name_2,
			req.body.referal_email,
			req.body.referal_email_2,
			req.body.referal_number,
			req.body.referal_number_2,
			req.body.resume,
		...
		req.session.error = {
			err_level: "danger",
			err_msg:
				"Failed to submit information. Please try again. Error: " +
					error.message || error,
		};
...
app.get("/information_1", (req, res) => {
	res.render("pages/information_1", {
		user: req.session.user,
		error: req.session.error,
	});
});
...
app.get("/api/jobs", async (req, res) => {
	...
	try {
		...
		if (requires_us_citizenship != null) {
			conditions.push(
				"requires_us_citizenship = $" + (conditions.length + 1)
			);
			...
			if (data) {
				...
				if (match) {
					...
					req.session.error = {
						err_level: "danger",
						err_msg:
							"Invalid Email or Password. To Register, click the Register button.",
					};
					res.redirect("/login");
				}
			} else {
...
app.get("/home", async (req, res) => {
	...
	try {
		const jobs = await fetch(
			"http://localhost:3000/api/jobs?limit=15&random=true"
		).then((res) => res.json());
...
app.get("/jobs", async (req, res) => {
	try {
		...
		if (ft_search === undefined) {
			jobs = await fetch("http://localhost:3000/api/jobs?limit=50").then(
				(res) => res.json()
			);
			...
			jobs = await fetch(
				`http://localhost:3000/api/jobs?ft_search=${ft_search}&limit=50`
			).then((res) => res.json());
			res.render("pages/jobBoard", {
				jobs,
				user: req.session.user,
	...
	db.one(checkIfUserOwnsApp, [
		...
		if (!user.exists) {
			...
			db.one(query, [req.params.applicationid])
				.then((app) => {
					res.render("pages/update-application", {
						user: req.session.user,
						app: app,
					});
				})
...
app.post("/job-applications/edit", (req, res) => {
	const query =
		"UPDATE applications SET name = $1, company = $2, industry = $3, description = $4, status = $5 WHERE appID = $6";
	const checkIfUserOwnsApp = `SELECT EXISTS(SELECT * FROM user_to_applications WHERE userID = $1 AND appID = $2)`;
...
app.get("/autofill", async (req, res) => {
	...
	const query = `SELECT * FROM personal_info WHERE user_id = ${req.session.user.userid};`;

	var data = await db.one(query);

	// resume BYTEA
	// Convert Resume to Base64
	// var base64 = Buffer.from(data.resume).toString("base64");

	var base64 = Buffer.from(data.resume).toString("base64");
	data.resume = base64;

	console.log(data);
	res.render("pages/autofill", { user: req.session.user, data: data });
});

// Handle Add Job Application
app.post("/job-applications/add", (req, res) => {
	const queryOne =
		"INSERT INTO applications (name, company, industry, description, status) VALUES ($1, $2, $3, $4, $5) RETURNING appID";
	...
	db.one(queryOne, [
		...
		.then((appid) => {
			const queryTwo =
				"INSERT INTO user_to_applications (userID, appID) VALUES ($1, $2)";
...
app.post("/job-applications/delete", (req, res) => {
	...
	const queryTwo =
		"DELETE FROM user_to_applications WHERE appID = $1 AND userID = $2";
	const checkIfUserOwnsApp = `SELECT EXISTS(SELECT * FROM user_to_applications WHERE userID = $1 AND appID = $2)`;
